...
const plotAxes = (xScale, yScale, xAxisName) => {
...
const plotActualCrimePerPopulationDensity = (data, predictorName) => {
	console.log(data);
    let disctrictNumExtent = d3.extent(data, (district) => { return district.ImportantPredictor; });
	...
	var errorBars = datapoints.enter()
        ...
        .attr("x1", (d) => disctrictNumScale(d.ImportantPredictor))
        .attr("y1", (d) => crimeScale(d.ActualCrime))
        .attr("x2", (d) => disctrictNumScale(d.ImportantPredictor))
        .attr("y2", (d) => crimeScale(d.ModeledCrime));

    var modeled = datapoints.enter()
        .append('circle')
        .attr('r', 2.5)
        .attr('cx', (d) => disctrictNumScale(d.ImportantPredictor))
    ...
    plotAxes(disctrictNumScale, crimeScale, predictorName);
    axesExist = true;
}
...
function update() {
    ...
    d3.csv("data/AggregateData/AggregateRelativePerArea.csv", parseLineRelative, function (error, data) {
		...
		for (var i = 1; i < betas.length; i++){
			if (betas[i] * selectedMeans[i-1] < 0){
				weightedBetas.push(betas[i] * selectedMeans[i-1] * -1);
			} else {
        ...
        let toPlot = crimeMatrix.map((district, i) => {
			...
			let impPredictor = district[iMax + 1];
            ...
            return {
				...
				ImportantPredictor: impPredictor,
                Error: error
            }
        });
        ...
        let sortedData = toPlot.sort((d1, d2) => {
			return d1.ImportantPredictor > d2.ImportantPredictor ? 1 //concat to end
				: d1.ImportantPredictor < d2.ImportantPredictor ? -1 //prepend to start
                    : 0; //equal
        });
        ...
        plotActualCrimePerPopulationDensity(sortedDataWithIndex, keys[iMax + 1]);
