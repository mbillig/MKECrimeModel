...
const calculateConfidenceIntervals = (crimeMatrix, model, observedIndex) => {
    ...
    var criticalValue = 2 * 2.58;
    let y = math.eval('crimeMatrix[:, ' + iActualsCol + ']', { crimeMatrix });
    y = [].concat.apply([], y);
    let A = math.eval('crimeMatrix[:, 1:' + nPredictorCols + ']', { crimeMatrix });
    let residuals = math.eval('model - y', { model, y });
    let MSres = math.eval('sum(residuals * residuals) / (n - 2)', { residuals, n })
    let unweighted = math.eval(`A[` + (observedIndex + 1) + `,:] * inv(A' * A) * A[` + (observedIndex + 1) + `,:]'`, { A });
    unweighted = [].concat.apply([], unweighted)[0];
    let standardError = math.eval('sqrt(MSres * unweighted)', { MSres, unweighted });
    let minConf = model[observedIndex] - criticalValue * standardError;
    let maxConf = model[observedIndex] + criticalValue * standardError;
...
const plotActualCrimePerPopulationDensity = (data) => {
    ...
    var confBars = datapoints.enter()
        .append("line")
        .style("stroke", "gray") // Add a color
        .style("stroke-width", "4")
        .attr("x1", (d) => disctrictNumScale(d.SortedOrder))
        .attr("y1", (d) => crimeScale(d.MaxConf))
        .attr("x2", (d) => disctrictNumScale(d.SortedOrder))
        .attr("y2", (d) => crimeScale(d.MinConf));
...
function update() {
    ...
    d3.csv("data/AggregateData/AggregateRelativePerArea.csv", parseLineRelative, function (error, data) {
        ...
        let confInterval = calculateConfidenceIntervals(crimeMatrix, modeledCrime, 0);
        ...
        let toPlot = crimeMatrixDistNum.map((district, i) => {
            ...
            let confMinAndMax = calculateConfidenceIntervals(crimeMatrix, modeledCrime, i);
            //console.log(confMinAndMax);
            ...
            return {
                ...
                Error: error,
                MinConf: Math.max(320, confMinAndMax[0]),
                MaxConf: confMinAndMax[2]
            }
        });
